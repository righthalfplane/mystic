...
intret copen(intret *iui, _fcd str)
{
        ...
        if(iu < 0 || iu >= MAXFILE){
            sprintf(buff,"copen: unit number %d out of range (0 < %d)",iu,MAXFILE);
            cerror(buff);
            return -1;
        }
...
intret ccreate(intret *iui, _fcd str)
{
        ...
        if(iu < 0 || iu >= MAXFILE){
            sprintf(buff,"ccreate: unit number %d out of range (0 < %d)",iu,MAXFILE);
            cerror(buff);
            return -1;
        }
...
{
        intret address,count;
        unsigned int ret;
        char buff[256];
        long int iu;

        if(!iui || !counti || !addressi || !data){
            sprintf(buff,"cwrite: NULL Pointer unit %p data %p count %p adress %p",
        ...
        if(iu < 0 || iu >= MAXFILE){
            sprintf(buff,"cwrite: unit number %ld out of range (0 < %d)",iu,MAXFILE);
            cerror(buff);
            return 1;
        }
...
*/
	...
	ret = fwrite(data,WORDSIZE,count,file[iu].fp);
        if(ret != count){
            file[iu].ierror=TRUE;
            sprintf(buff,"cwrite: write error\n file %s unit %ld address %lld\n words requested: %lld words written: %ld",
                    file[iu].name,iu,address,count,(long)ret);
            cerror(buff);
            return 1;
        }
...
intret cread(intret *iui,unsigned char *data,intret *counti,intret *addressi)
{
        ...
        if(!iui || !data || !counti || !addressi){
            sprintf(buff,"cread: NULL Pointer unit %p data %p count %p adress %p",
                    iui,data,counti,addressi);
            cerror(buff);
            return 1;
        }
        ...
        if(iu < 0 || iu >= MAXFILE){
            sprintf(buff,"cread: unit number %ld out of range (0 < %d)",iu,MAXFILE);
            cerror(buff);
            return 1;
        }
...
*/
        ...
        if(ret != count){
            file[iu].ierror=TRUE;
            sprintf(buff,"cread: read error\n file %s unit %ld address %lld\n words requested: %lld words read: %ld",
                    file[iu].name,iu,address,count,(long)ret);
            cerror(buff);
            return 1;
        }
...
intret cclose(intret *iui)
{
        ...
        if(iu < 0 || iu >= MAXFILE){
            sprintf(buff,"cclose: unit number %ld out of range (0 < %d)",iu,MAXFILE);
            cerror(buff);
            return -1;
        }
...
intret cclear(intret *iui)
{
        ...
        if(iu < 0 || iu >= MAXFILE){
            sprintf(buff,"cclear: unit number %ld out of range (0 < %d)",iu,MAXFILE);
            cerror(buff);
            return -1;
        }
