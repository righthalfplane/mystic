...
for (;;){
    ...
    if(TYP != 1 && TYP != 15)fout << boost::format("%d %d %d %d ") % ne % TYP % tags % reg;
    
    for (int ntag=1; ntag<tags; ntag++) {
        boost::advance(tok_iter, 1);
        int i=str2num<int>(*tok_iter);
             if(TYP != 1 && TYP != 15)fout << boost::format("%d ") % i;
	    }
	    
    fprintf(stderr,"TYP %ld\n",(long)TYP);
