...
showPtr showStart(char *name,char *mode)
{
	...
	s=(showPtr)cMalloc(sizeof(struct SHOW));
...
int showTime(showPtr s,double timei)
{
	if(!s)return 1;
...
int showEnd(showPtr s)
{
	...
	if(showLong(s,&s->stressMax,1L))goto ErrorOut;
	if(showFloat(s,&s->time,1L))goto ErrorOut;
	if(showLong(s,&s->velCount,1L))goto ErrorOut;
	...
	if(s->velCount){
	    if(showFloat(s,s->xvel,s->velCount))goto ErrorOut;
	    if(showFloat(s,s->yvel,s->velCount))goto ErrorOut;
	    if(showFloat(s,s->zvel,s->velCount))goto ErrorOut;
	}
	...
	if(s->stressCount){
	    if(showFloat(s,s->stress,s->stressCount*s->stressMax))goto ErrorOut;
	}
...
int showFloat(showPtr s,double *x,unsigned long nn)
{
	...
	if(s->pc){
	    for(n=0;n<nn;++n){
            ...
            c[6]=lp[1];
            c[7]=lp[0];
            if(showWrite(s,(unsigned char *)c,8L))return 1;
	    }
	}else{
...
int showVelocities(showPtr s,long nodeCount,double *dx,double *dy,double *dz)
{
	double *sx,*sy,*sz;
	char buff[256];
	long n;
	...
	if(!sx || !sy || !sz){
	    ...
	    sprintf(buff,"showVelocities Out Of Memory (Requested %ld)",
	           nodeCount*sizeof(double)*3);
	    Warning(buff);
	    return 1;
	}
...
int showStress(showPtr s,long nodeCount,double *stress)
{
	...
	sx=(double *)cMalloc(nodeCount*sizeof(double)*s->stressMax);

	if(!sx){

	    sprintf(buff,"showDisplacements Out Of Memory (Requested %ld)",
	            nodeCount*sizeof(double)*s->stressMax);
	    Warning(buff);
	    return 1;
	}

	for(n=0;n<nodeCount*s->stressMax;++n){
	    sx[n]=stress[n];
	}
