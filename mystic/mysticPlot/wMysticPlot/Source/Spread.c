...
static void tDoControls(struct SpreadStruct *Spread,Point where);
...
static int ScrollRect(Rect *etr, int dx, int dy,struct Icon *myIcon)
{
	...
	ScrollDC(myIcon->hdc,dx,dy,NULL,&r,NULL,NULL);
	return 0;
}
/*
static int ScrollRect(Rect *etr, int dx, int dy,struct Icon *myIcon)
...
*/
static int SetRect2(Rect *r,int left,int top,int right,int bottom)
{
	if(!r)return 1;
	r->left=left;
	r->top=top;
		                       ...
		                       struct Icon *myIcon)
{
	...
	myIcon->VControl=Spread->VControl=control;
	...
	control=ControlCreate(myIcon,&controlRect,"controlName",
		  ...
		  0L,0L,20,0L);
	if(!control)goto ErrorOut;
	myIcon->HControl=Spread->HControl=control;
...
/*	controlPtr control; */
	...
	if(uPtInRect(w,&r)){
		...
		if(!(FindControl(myIcon.myevent,myIcon.mywindow,&control))){
			old=uGetControlValue(Spread->HControl);
		}else{
...
{	
	...
	DumpText(wp,wp->XOff - wp->oXOff,wp->YOff - wp->oYOff);
	wp->oXOff=wp->XOff;
	wp->oYOff=wp->YOff;
	return 0;
}
...
{	
		...
		if(myIcon && (myIcon->WindowType == TableType)){
...
static void DoClick(struct SpreadStruct *wp,Point p,int extend)
{
	...
	Select=wp->Select;
		while (uStillDown(wp->myIcon)) {
			...
			where->v=pt.y;
			r=uRectConvert(&tr);
			if(Select.bottom != wp->Select.bottom || Select.top != wp->Select.top ||
			   Select.left != wp->Select.left || Select.right != wp->Select.right){

				DumpText(wp, 0, 0);
				Select=wp->Select;
			}
			if (!(itwas=uPtInRect(pt,&r))) {			/* Is outside of window, scroll */
...
static void DrawCell(struct SpreadStruct *wp,int x,int y)
{
	char out[256];
	char *f;
	int i,j,widex,widey,nc,widestring;
	Rect tr,tf,viewRect;
...
static void DumpText(struct SpreadStruct *wpp,int dh,int dv)
{
	...
	// TextFace(bold);
	if (!dv) {
		SetRect2(&etr, viewRect.left, viewRect.top, viewRect.right, viewRect.top+widey );
		r=uRectConvert(&etr);
		uEraseRect(&r,wp.myIcon);
		for (i=0; i<cx; i++) {
		    if(wp.pListColumns){
	...
	if (!dh) {
		SetRect2(&etr, viewRect.left, viewRect.top, viewRect.left+wp.RowOffSet, viewRect.bottom );
		r=uRectConvert(&etr);
		uEraseRect(&r,wp.myIcon);
		for (i=0; i<cy; i++) {
		    if(wp.pListRows){
	...
	if (dh || dv) {						
	    ...
	    SetRect2(&etr, viewRect.left+wp.RowOffSet, viewRect.top+widey, viewRect.left+wp.RowOffSet+widex*wp.XScreen, viewRect.top+widey+widey*wp.YScreen );
...
static void ListSet2(char *out,int x,int y,struct SpreadStruct *wp)
{
	...
	if(wp->DataFloat){
	    wp->DataFloat[x+y*wp->XCount]=(float)atof(out);
	}
}
static void ListRows(char *out,int i,struct SpreadStruct *wp)
{
