...
static int doSage3DTracerPlotLineData(struct FileInfo2 *Files,int *list,int nc)
{
	...
	if(pio->TracerPlotCount && pio->TracerPlotList){
	    ...
	    TracerPlotList=cMalloc((pio->TracerPlotCount+nn)*sizeof(int),8260);
	    if(!TracerPlotList){
	    ...
	    pio->TracerPlotList=cMalloc(nc*sizeof(int),0261);
...
static int doSage3DSavePlotLineData(struct FileInfo2 *Files,int *list,int nc)
{
	...
	if(pio->LaserPlotCount && pio->LaserPlotList){
	    ...
	    LaserPlotList=cMalloc((pio->LaserPlotCount+nn)*sizeof(int),8260);
	    if(!LaserPlotList){
	    ...
	    pio->LaserPlotList=cMalloc(nc*sizeof(int),8281);
