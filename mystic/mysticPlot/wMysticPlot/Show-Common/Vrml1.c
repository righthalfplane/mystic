...
static int CVrmlLoad(CVrmlPtr s,long itIs);		 
...
CObjectPtr uVrmlWrite(struct CObjectStruct *o,FILE8 *inOut,CObjectListPtr l)
{
	...
	if(!bp){
		...
		if(uPref.Files){
		    ...
		    uPref.Files=NULL;
			if(b.Files){
				bp=CVrmlCreate(l->scene,b.Files);
				if(!bp)goto  ErrorOut;
				bp->Files=b.Files;
		        b.Files=NULL;
		    }else{
...
static int CVrmlKill(CObject *o)
{
	CVrmlPtr s=(CVrmlPtr)o;
...
CVrmlPtr CVrmlCreate(struct Scene *scene,struct FileInfo2 *Files)
{
	...
	s=(CVrmlPtr)cMalloc(sizeof(CVrml),8540);
	if(!s)return (CVrmlPtr)NULL;
	zerol((char *)s,sizeof(CVrml));
...
static CObjectPtr CVrmlDuplicate(CObject *b)
{
	...
	sdsNew=(CVrmlPtr)cMalloc(sizeof(CVrml),8541);
	if(!sdsNew)return (CObjectPtr)NULL;

	*sdsNew = *s;
	
	sdsNew->vrml=duplicateVRML(s->vrml);
