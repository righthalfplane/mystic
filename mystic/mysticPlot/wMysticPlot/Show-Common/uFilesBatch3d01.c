...
static int doSage3DCheckBlocks(struct FileInfo2 *Files,long CurrentFrame);
...
double *buildBlockSDS3D(struct FileInfo2 *Files,long CurrentFrame);
...
*/	
	if(GradientType == 0){
		...
		if(pio->vectorLength < xBlockSize){
			...
			pio->vectorz=(double *)cRealloc((char *)pio->vectorz,xBlockSize*sizeof(double),9183);
		...
		if(doBlockGradientsGet(Files,start,BlockSize,CurrentFrame,
							   pio->lo,nLow,pio->hi,nHigh,pio->vectorz,pio->vector.vectorzName,0L))goto ErrorOut;
...
static int pioDrawStream3d(struct FileInfo2 *Files,struct screenData *ss,struct dRange *r)
{
	...
	if(!stream){
	    WarningBatch("pioDrawStream2d Blocks Out of List Memory\n");
	    goto ErrorOut;
	}
	...
	if(doSage3DCheckBlocks(Files,CurrentFrame))goto ErrorOut;
...
*/	
	...
	if(BlocksStart(Files,B_3D_STANDARD))goto ErrorOut;
	...
	do{
		...
		if(BlocksRead(Files,B_3D_STANDARD,nff,BlockSize,CurrentFrame))goto ErrorOut;
...
int doSage3DGetAreaDataBlock(struct FileInfo2 *Files,struct areadata *ai)
{
	...
	if(ai->zsize <= 1){
	    ...
	    ai->data=buildBlockSDS3D(Files,CurrentFrame);
	    
	    if(!ai->data){
	        WarningBatch("doSage3DGetAreaData Error in buildBlockSDS3D\n");
	        return 1;
	    }else{
	        return 0;
	    }
...
*/		
	if(nHigh){
		...
		while(1){
			...
			if(++nl >=  nHigh){
				sprintf(WarningBuff,"Frame %ld : doBlockGradientsGet Not All High data not found\n",CurrentFrame);
				WarningBatch(WarningBuff);
				break;
			}
			nf=nl;   /* skip to next needed Item */
...
/*
	...
	*/
	...
	nHighs=nHigh;
	/*
	sprintf(WarningBuff,"doBlockGradients BlockSize %ld nLows %ld nHighs %ld\n",BlockSize,nLows,nHighs);
	WarningBatch(WarningBuff);
	*/
/*
	value=(double *)cMalloc(sizeof(double)*xBlockSize,19727);
...
int doSage3DGetDataBlocks(struct FileInfo2 *Files,long CurrentFrame,struct SetFrameData  *sd)
{
	...
	if(sd->type != FRAME_DATA_FLOAT){
	    if(sd->type == FRAME_DATA_RGB){
		    ...
		    sprintf(WarningBuff,"doSage3DGetDataBlocks - Only Returns FRAME_DATA_FLOAT and FRAME_DATA_RASTER\n");
		    WarningBatch(WarningBuff);
		    goto ErrorOut;
		}
...
*/
    ...
    sd->data=cMalloc(length*sizeof(double),9206);
    if(!sd->data){
    	sprintf(WarningBuff,"doSage3DGetDataBlocks - Out of Memory\n");
    	WarningBatch(WarningBuff);
		goto ErrorOut;
    }
    ...
    if(sd->type == FRAME_DATA_FLOAT){
	    ...
	    if(getPlotData(Files,sout,CurrentFrame)){
		    sprintf(WarningBuff,"doSage3DGetDataBlocks - getPlotData Error\n");
		    WarningBatch(WarningBuff);
			goto ErrorOut;
	    }	
