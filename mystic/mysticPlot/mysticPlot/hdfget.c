...
static struct hdfRaster Raster;

static long int countHDF;
...
int writeHDF(void)
{
	...
	if(!Raster.buffer)return 0;
	
	buff=(unsigned char *)cMalloc(Raster.xsize*Raster.ysize+4094,2098);
	if(!buff)goto ErrorOut;
		
	UnPack(buff,(unsigned char *)Raster.buffer,(long)Raster.xsize*Raster.ysize);
	
	if(countHDF == 0){
	   mstrncpy(filepath,Raster.name,sizeof(filepath));
	   mstrncat(filepath,".hdf",sizeof(filepath));
	}
	
	limit.command=(unsigned char *)&Raster.limit[0];
	
	limit.iGot=(Raster.limit[0] > 0);
	
	mstrncpy(filepathname,directory,sizeof(filepathname));
	mstrncat(filepathname,"/",sizeof(filepathname));
	mstrncat(filepathname,filepath,sizeof(filepathname));
		
	hdfOut(filepathname,Raster.name,Raster.pal,Raster.xsize,Raster.ysize,(char *)buff,countHDF,countHDF == 0,&limit);
	
	countHDF++;
	
ErrorOut:
	if(buff)cFree((char *)buff);
	if(Raster.buffer)cFree((char *)Raster.buffer);
	Raster.buffer=NULL;
	return 0;
}
...
int main(int argc,char *argv[])
{
	...
	zerol((char *)&Raster,sizeof(struct hdfRaster));
	
	getcwd(directory,sizeof(directory));
	
	getPalette(12,NULL,&Raster.pal[0]);

	fprintf(stderr,"************************************************\n");
	fprintf(stderr,"**  hdfget 17.0a0 - COPYRIGHT SAIC 1993-2021. **\n");
	fprintf(stderr,"************************************************\n");
	...
	for(n=1;n<argc;++n){
	    if(!strcmp(argv[n],"-debug")){
		   Debug = 1;
	    }else if(!strcmp(argv[n],"-openwith")){
	         OpenWithApplication=argv[++n];
...
int Usage(void)
{
	...
	fprintf(stderr,"hdfget 10.11.128.54:3600 -openwith /Applications/mShow.app \n");
	fprintf(stderr,"                                                           \n");
	fprintf(stderr,"         [options]                                         \n");
	fprintf(stderr,"                                                           \n");
	fprintf(stderr,"     -debug    (turn on debug print)                       \n");
	fprintf(stderr,"     -spaces   (remove spaces from names)                  \n");
	fprintf(stderr,"     -openwith (set openwith application on Macontosh)     \n");
	fprintf(stderr,"                                                           \n");
...
int ListenSocket(int clientSocket)
{
	...
	fprintf(stderr,"******************************************************\n");
	fprintf(stderr,"**  hdfget 17.0a0 - COPYRIGHT SAIC 1993-2021. Start **\n");
	fprintf(stderr,"******************************************************\n");
	...
	fprintf(stderr,"******************************************************\n");
	fprintf(stderr,"**  hdfget 17.0a0 - COPYRIGHT SAIC 1993-2021. Done  **\n");
	fprintf(stderr,"******************************************************\n");
...
int doH8CM(int clientSocket)
{
	...
	writeHDF();
	if(getname(clientSocket,&Raster.name[0]))return 1;
	
	if(getInt(clientSocket,&Raster.xsize))goto Error;
	if(getInt(clientSocket,&Raster.ysize))goto Error;
	if(getLong(clientSocket,&Raster.length))goto Error;
	
	if(Debug){
		printf("name %s xsize %d ysize %d length %ld\n",Raster.name,Raster.xsize,
		   Raster.ysize,Raster.length);
		fflush(stdout);
	}
	
	
	if(Raster.buffer)cFree((char *)Raster.buffer);
	Raster.buffer=(char *)cMalloc(Raster.length,1889);
	if(!Raster.buffer)goto Error;
	
	if(readString(clientSocket,Raster.buffer,Raster.length))goto Error;
...
int doLIMI(int clientSocket)
{
	...
	if(getname(clientSocket,&Raster.limit[0]))return 1;
...
int doPAL8(int clientSocket)
{
	...
	if(readString(clientSocket,&Raster.pal[0],256*3))return 1;
...
int relayEnd(int clientSocket,int toServerSocket,int buffsize)
{
	...
	fprintf(stderr,"****************************************************\n");
	fprintf(stderr,"**  relayEnd 16.9 - COPYRIGHT SAIC 2021.    Start **\n");
	fprintf(stderr,"****************************************************\n");
...
ErrorOut:
        ...
        printf("%ld Seconds To Receive %ld Bytes (%ld Bytes/s)\n",
                 total,Bytes,(Bytes/total));
	fprintf(stderr,"****************************************************\n");
	fprintf(stderr,"**  relayEnd 16.9 - COPYRIGHT SAIC 2021.     Done **\n");
	fprintf(stderr,"****************************************************\n");
...
int relaySocket(int clientSocket,int toServerSocket,int buffsize)
{
	...
	fprintf(stderr,"****************************************************\n");
	fprintf(stderr,"**  relay 16.9 - COPYRIGHT SAIC 2021.       Start **\n");
	fprintf(stderr,"****************************************************\n");
...
ErrorOut:
	...
	if(!total)total=1;
        fprintf(stderr,"%ld Seconds To Receive %ld Bytes (%ld Bytes/s)\n",
                 total,Bytes,(Bytes/total));
	fprintf(stderr,"****************************************************\n");
	fprintf(stderr,"**  relay 16.9 - COPYRIGHT SAIC 2021.       Done  **\n");
	fprintf(stderr,"****************************************************\n");
