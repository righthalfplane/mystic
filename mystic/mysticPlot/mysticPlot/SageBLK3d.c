...
static IconPtr OpenPIO3DFiles(struct FileInfo2 *Files);
...
static IconPtr OpenPIO3DFiles(struct FileInfo2 *Files)
{
	...
	uRect Rect1={40,40,548,462};
		...
		if(!(myIcon=uCreateIcon(Pio3dBklType,&Rect1))){
			WarningBatch("OpenPIO3DBLKFiles uCreateIcon Failed\n");
			goto ErrorOut;
		}
		
		sprintf(buff,"PIO3DBLK_%03d",Count++);
	    ...
	    if(!IsShift()){
	    	doUniversalSageBLK3D(myIcon,universal_GetDefaults,NULL);
	    	FlushCurrentPort();
	    }
...
static int doLoadImages(IconPtr myIcon,struct FileList *Files)
{
	...
	int TypeList[]={SAGE3D_DISK,SDS3D_MEMORY,SDS3D_DISK,0};
	char buff[256];
	pio3dPtr t;
	...
	if(!t->initFile){
	    doUniversalSageBLK3D(myIcon,universal_SetDefaults,NULL);
	    initPIO3d(myIcon);
	    doUniversalSageBLK3D(myIcon,universal_GetDefaults,NULL);
		doFrameImage(myIcon);
	}
...
static int loadFrame(IconPtr myIcon,long CurrentFrame)
{
	...
	myIcon->limits = sd->limits;
	if(sd->limits.pioName && sd->limits.pioName[0]){
...
int doUniversalSageBLK3D(IconPtr myIcon,int flag,FILE8 *inOut)
{
	...
	static struct universalTableStruct Global2 = {
		"SagePIO3DBLK parameters",universal_WindowSageBLK3D,
		(void *)&mm,sizeof(mm),
		NULL,0L,							
	...
	s=NULL;
	if(myIcon && (myIcon->WindowType == Pio3dBklType)){
		s=(pio3dPtr)myIcon->DWindow;
	}else{
	    myIcon=NULL;
	}
	...
	if(flag == universal_SetDefaults){
		...
		sprintf(WarningBuff,"doUniversalSageBLK3D unknown flag %d\n",flag );
		WarningBatch(WarningBuff);
		goto ErrorOut;
	}
	ret=0;
